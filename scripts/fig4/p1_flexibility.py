...
amino_p1 = "Mpro-x0434_0A"
ugi_p1 = "Mpro-P0053_0A"
quin_p1 = "Mpro-x10419_0A"
benzo_p1 = "Mpro-x12423_0A"

fragments = [amino_p1, ugi_p1, quin_p1, benzo_p1]

# Load each fragment: ligand + apo protein
for i, fragment in enumerate(fragments):
    cmd.load(path + f"{fragment}/" + f"{fragment}.sdf", f"p1-{i}-{fragment}-ligand")
    cmd.load(
        path + f"{fragment}/" + f"{fragment}_apo-desolv.pdb",
        f"p1-{i}-{fragment}-protein",
    )

# Sort out colours for each series
cmd.color("wheat", f"p1-0-*")  # amino
cmd.color("palegreen", f"p1-1-*")  # ugi
cmd.color("violet", f"p1-2-*")  # quin
cmd.color("deepolive", f"p1-3-*")  # benzo
...
# Create images
print("Creating images for P1...")
for i, fragment in zip([0, 1, 2, 3], fragments):
    fragment_key = {0: "amino", 1:"ugi", 2: "quin", 3: "benzo"}
    print(fragment_key[i], fragment)

    cmd.enable(f"p1-{i}-{fragment}-ligand")
    cmd.enable(f"p1-{i}-{fragment}-protein")

    cmd.ray(720, 720)
    cmd.png(f"./p1_flex_{fragment_key[i]}_{fragment}.png", dpi=300)

    cmd.disable(f"p1-{i}-{fragment}-ligand")
    cmd.disable(f"p1-{i}-{fragment}-protein")
