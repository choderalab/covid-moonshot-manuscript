...
for fragment in fragments:
    cmd.load(path + f"{fragment}/" + f"{fragment}.sdf", f"quinolones-{fragment}-ligand")
    cmd.load(
        path + f"{fragment}/" + f"{fragment}_apo-desolv.pdb",
        f"quinolones-{fragment}-protein",
    )
cmd.color("violet", f"quinolones-*")
util.cnc(f"quinolones-*")
...
cmd.show("surface", f"quinolones-Mpro-x3303_0A-protein and not hydrogen")
cmd.disable("*-protein")
cmd.enable("quinolones-Mpro-x3303_0A-protein and not hydrogen")
